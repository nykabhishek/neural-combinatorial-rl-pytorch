...
def test3():
    mat = scio.loadmat('targCords.mat')
    print(mat['targ_cords'])
    targ_cords = np.array(list(mat['targ_cords']))
    no_targets = targ_cords.shape[0]
    # print(targ_cords.shape[0])
    n_disc=12
    infn = 20000
    Mnb = 10000

    # use concorde or lkh in quotes
    solver = "lkh"  # or "concorde" 

    thVec = np.linspace(0,2*math.pi-2*math.pi/n_disc,n_disc)
    # thVec = np.linspace(0,360-360/n_disc,n_disc)
    # print(thVec)

    dcost_orig = np.zeros((no_targets*n_disc, no_targets*n_disc))
    print(dcost_orig.shape)
    # print(np.arange(no_targets))

    iCm=0; jCm=0;
    for i in arange(no_targets):
        for j in arange(no_targets):
            
            [a_xcord, a_ycord] = targ_cords[i]
            # a_ycord = targ_cords[i]
            [b_xcord, b_ycord] = targ_cords[j]
            # b_ycord = targ_cords[1]


            # if (i==j):
            if([a_xcord, a_ycord]==[b_xcord, b_ycord]):
                iCm = n_disc*(i)
                for a_ang in thVec:
                    jCm = n_disc*(j)
                    iCm = iCm+1
                ...
                for a_ang in thVec:
                    ...
                    for a_ang in thVec:
                        jCm = jCm+1
                        dubins = DubinsPath([a_xcord,a_ycord,a_ang], [b_xcord,b_ycord,b_ang], 100.0)
                        # temp = dubins_orig(a_xcord,a_ycord,a_ang,b_xcord,b_ycord,b_ang,rho,0)
                        dubins.calc_paths()
                        path, cost = dubins.get_shortest_path()
                        dcost_orig[iCm-1,jCm-1] = cost
    # np.savetxt('matrix.txt', dcost_orig)
    print(dcost_orig)

    sets_sizes = n_disc*np.ones((no_targets,1))
    print(sets_sizes)
    cost_trans = octave.nbtranscost(dcost_orig, n_disc, no_targets)
    print(cost_trans)
...
def test4():
    ...
    draw_point(start)
    draw_point(end)
    xs, ys = DubinsPath.gen_path(start, path, 2)
    for i in range(3):
        plt.plot(xs[i], ys[i])
    plt.axis("equal")
